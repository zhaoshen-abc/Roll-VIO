...
void ROS1Visualizer::visualize_odometry(double timestamp) {
...
#ifdef ROLL_STATE_OUTPUT
  t_roll = _app->compute_roll_trans(state_plus.block(0, 0, 4, 1), timestamp);
  state_plus(4) = t_roll(0);
  state_plus(5) = t_roll(1);
  state_plus(6) = t_roll(2);
#endif
  ...
  if (pub_odomimu.getNumSubscribers() != 0) {
    ...
    odomIinM.pose.pose.position.x = t_roll(0);
    odomIinM.pose.pose.position.y = t_roll(1);
    odomIinM.pose.pose.position.z = t_roll(2);
...
#ifdef ROLL_STATE_OUTPUT
  Eigen::Matrix<double, 3, 1> t = _app->compute_roll_trans(state->_imu->quat(), timestamp_inI);
  poseIinM.pose.pose.position.x = t(0);
  poseIinM.pose.pose.position.y = t(1);
  poseIinM.pose.pose.position.z = t(2);
#endif
  // Finally set the covariance in the message (in the order position then orientation as per ros convention)
...
void ROS1Visualizer::publish_groundtruth() {
  ...
  Eigen::Vector3d v1 = quat_diff.block(0, 0, 3, 1);
  Eigen::Vector3d v2 = covariance.block(0, 0, 3, 3).inverse() * 2 * quat_diff.block(0, 0, 3, 1);
  double ori_nees = 2 * v1.dot(v2);
