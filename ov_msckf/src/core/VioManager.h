...
public:
  ...
  // 旋转直接使用state，平移用滚动模型算
  Eigen::Matrix<double, 3, 1> compute_roll_trans(Eigen::Matrix<double, 4, 1> q, double time);
...
protected:
  ...
  // 滚动相关变量
  bool first_compute_roll = false;
  Eigen::Quaterniond q_last;
  double time_last;
  double R = 1; // 球半径
  Eigen::Matrix4d Calib_IMUtoOrigin;
};
