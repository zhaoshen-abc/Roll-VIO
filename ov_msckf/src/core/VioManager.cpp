...
Eigen::Matrix<double, 3, 1> VioManager::compute_roll_trans(Eigen::Matrix<double, 4, 1> q, double time) {
  Eigen::Matrix<double, 3, 1> t = Eigen::Matrix<double, 3, 1>::Zero();
  if (first_compute_roll == false) {
    time_last = time;
    q_last = Eigen::Quaterniond(q(0), q(1), q(2), q(3));
    first_compute_roll = true;
    return t;
  }
  double dt = time - time_last;
  Eigen::Quaterniond quat(q(0), q(1), q(2), q(3));
  Eigen::Matrix<double, 3, 1> w = 2 * (q_last.conjugate() * quat).vec() ;
  Eigen::Vector3d ground(0, 0, 1);
  t = R * w.cross(ground);
  return t;
}
