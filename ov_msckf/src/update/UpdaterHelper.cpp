...
#define ROLL
#ifdef ROLL
      // Rcw(2, 2) 为cos(theta)，相机z轴与地面z轴夹角的余弦值，越接近0,表示相机视角越接近水平，权重越大
      Eigen::Vector3d z(0, 0, 1);
      Eigen::MatrixXd R_CtoG = R_ItoC * R_GtoIi;
      // Eigen::Vector3d bearing = R_CtoG * z;
      Eigen::Vector3d bearing = p_FinCi.normalized();
      double cos_theta = bearing.dot(z);
      double score = 1 - fabs(cos_theta);
      // std::cout << "score: " << score << std::endl;
      assert(score > 0.0);
      res.block(2 * c, 0, 2, 1) = score * res.block(2 * c, 0, 2, 1);
      H_f.block(2 * c, 0, 2, H_f.cols()) = score * H_f.block(2 * c, 0, 2, H_f.cols());
      H_x.block(2 * c, 0, 2, H_x.cols()) = score * H_x.block(2 * c, 0, 2, H_x.cols());
#endif
